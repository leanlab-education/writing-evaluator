...
### Concede-or-escalate reconciliation (2026-08-11; softened to a confirmation 2026-08-17)

On a discrepancy, recording your **own** original value as the final means the pair agreed your score was right — a legitimate outcome, but also the shape of two silent failures: a no-discussion "reconciliation" where the second scorer re-picks their own value out of habit, and a 2-point-scale misclick. So the reconcile UI gates it behind a **confirmation dialog** ("Record your own original score? … only if you and your partner discussed it and agreed") before the value is applied. Confirm to proceed; cancel to let your partner record it or escalate (Luofan, 2026-07-29).

- **Why it's a confirmation and not a hard block:** the original rule forbade recording your own original outright (partner had to record it, or you escalate). That **deadlocked two-way concessions** — when each partner concedes a *different* criterion of the same item, one atomic save must record the submitter's own original on one row and the partner's on the other, so *neither* annotator could save the item alone. Jonathan/Luofan hit this on Batch 45 (2026-08-17). The confirmation removes the deadlock while keeping a deliberate speed bump against the two accidental cases.
- **Client-only, not server-enforced.** The gate lives in `reconcile-client.tsx` (`pendingSelfOriginal` dialog). The reconcile route no longer 403s on a self-original save; the old pure rule module (`reconcile-concession.ts`) was **removed**. The reconcile write still runs in one serializable transaction (P2034 → 409 on concurrent partner edits).
- The confirmation only fires on the **first** recording of a discrepancy (`persistedFinals[key] == null`) and only for the submitter's own original value. It does **not** fire on: agreed dimensions, a submitter with no original raw score (admins outside the pair), or editing an **already-persisted** final (that path uses the separate "change an already-reconciled score?" warning and may move to any value). Comparisons use the **persisted** final, not the local unsaved selection.
- **`Score.reconciledById`** still records who actually performed each reconciliation (pair-reconcile and adjudicate paths), stamped only when the final value changes. `userId` on a reconciled row is the release-owner storage slot, NOT the actor. Null = pre-2026-08 row or system auto-reconcile.
