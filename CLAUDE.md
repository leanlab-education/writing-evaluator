...
- It deliberately does **not** fire on: agreed dimensions (re-sent on every save), no-op resaves of an existing final, or submitters with no original raw score (admins). Comparisons use the **persisted** final, not the local unsaved selection.
